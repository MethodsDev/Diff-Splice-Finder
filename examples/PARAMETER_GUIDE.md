...
- `--site_depth_offsets` or `--site_depth_bam_list`: raw site-depth denominator matrix, or BAMs used to compute it
...
python3 util/count_introns_from_bam.py \
    --genome_fa reference.fa \
    --bam sample1.bam \
    --target_introns discovery.matrix > sample1.targeted.introns
...
Alternatively, compute offsets from a BAM list:
