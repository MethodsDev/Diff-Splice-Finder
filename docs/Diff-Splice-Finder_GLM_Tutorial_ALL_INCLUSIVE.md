...
The goal is conceptual understanding rather than mathematical rigor. The emphasis is on *why* the model works and *how* to interpret its coefficients biologically.
...
Raw intron counts alone cannot answer this question, because:

- gene expression may increase or decrease, affecting all introns together
- sequencing depth varies across samples
- long reads may contribute evidence to multiple introns

Splicing is therefore **compositional**: introns compete for usage within a local context.
...
Introns are grouped into **local splicing clusters**:

- **donor clusters**: introns sharing the same 5′ splice site
- **acceptor clusters**: introns sharing the same 3′ splice site

For each cluster `C` and sample `s`, we compute:

- `T[C, s]` = total intron-support reads in that cluster and sample  
  (the sum of counts across all introns in the cluster)
